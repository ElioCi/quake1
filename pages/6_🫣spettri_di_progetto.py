...
st.title("Spettri di risposta di progetto")
...
if 'fattoreS' not in st.session_state:
    st.session_state.fattoreS = 1.0
if 'fattoreCompVert' not in st.session_state:
    st.session_state.fattoreCompVert = 1.5


col1, col2, col3 = st.columns([1,2,1])
q0 = col1.number_input("Fattore di struttura q0", min_value= 1.0, value= st.session_state.fattoreS, step = 0.1)  

st.session_state.fattoreS = q0

Regolare = col2.radio("Struttura regolare in altezza ?", options= ["Si", "No"]) 

qv = col1.number_input("Fattore comp. vert. qv", min_value= 1.0, value= st.session_state.fattoreCompVert, step = 0.1) 

st.session_state.fattoreCompVert = qv
...
#scrivi il fattore di struttura considerato q
col3.markdown(f"<p style='margin-top:35px; color: white; background-color:green; text-align: center'> ➩ &nbsp &nbsp  q = {q: .2f}</p>", unsafe_allow_html=True)
col3.markdown(f"<p style='margin-top:35px; color: white; background-color:green; text-align: center'> ➩ &nbsp &nbsp  ηv = {etav: .2f}</p>", unsafe_allow_html=True)
...
av_values = [fvert(Tv, TBv, TCv, TDv, ag_sl, Sv, etav, F0_sl, Fv) for Tv in Tv_values]
...
figv.add_trace(go.Scatter(
    x=[TBv, TCv, TDv], 
    y=[fvert(TBv, TBv, TCv, TDv, ag_sl, Sv, etav, F0_sl, Fv), 
       fvert(TCv, TBv, TCv, TDv, ag_sl, Sv, etav, F0_sl, Fv), 
       fvert(TDv, TBv, TCv, TDv, ag_sl, Sv, etav, F0_sl, Fv)], 
    mode="markers", 
    marker=dict(color="red", size=10), 
    name="Punti caratteristici"
))
...
for nomev, valorev in zip(["TBv", "TCv", "TDv"], [TBv, TCv, TDv]):
    figv.add_annotation(
        x=valorev, 
        y=fvert(valorev, TBv, TCv, TDv, ag_sl, Sv, etav, F0_sl, Fv),  
        text=nomev,  
        showarrow=True,  
        arrowhead=2,  
...
# Aggiorna solo la colonna 'q' mantenendo inalterate le altre colonne
df_existing['q'] = q  
df_existing['etav'] = etav 
...
new_data = {"PH": 2, "q": q, "etav": etav}
# Aggiornamento delle sole chiavi presenti in new_data
st.session_state.input_backup.update(new_data)

# Aggiungere la nuova serie come seconda riga
df_inputBackup = pd.read_csv("files/inputBackup.csv")

df_inputBackup['JA'] = df_inputBackup['JA'].astype(str)
df_inputBackup.loc[1] = st.session_state['input_backup']
#df_inputBackup = pd.DataFrame(st.session_state['input_backup'], index = [1])
df_inputBackup.to_csv("files/inputBackup.csv", index=False)
#st.dataframe(df_inputBackup, hide_index= True)


#print (st.session_state.input_backup)
